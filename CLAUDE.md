...
- 선택 텍스트를 공백/한글 경계로 **토큰화**(라틴 조각 단위)해 토큰마다 매핑 비율(`mapRatio`)과
  조합 성공률(`composeRatio`)을 **개별** 계산하고, 그 중 최댓값을 선택 전체의 신뢰도로 쓴다
  (2026-09 재설계, 아래 "대량 fuzz 검증" 참조). 실제 영어 단어일 경우 조합 실패율이 높아 오탐 방지.
- **영어 상용어 스톱리스트(필수)**: 두벌식에서 자음-모음-자음 배열이 되는 흔한 영단어는 한글로
  100% 조합돼(the→솓, and→뭉, for→랙, with→쟈소, work→재가) 조합률만으로는 최고 신뢰도가 나온다.
  길이·음절수 같은 구조 신호로는 진짜 한영타(dkssud=안녕)와 분리되지 않으므로 토큰이
  `ENGLISH_STOPWORDS`(= `ENGLISH_STOPWORDS_BASE` ~150개 + 대량 fuzz 검증으로 추출한
  `ENGLISH_STOPWORDS_FREQUENT` 626개, 아래 참조)에 정확히 일치하면 그 토큰만 후보에서 제외한다
  — 선택이 상용어(들)로만 이뤄졌으면 자동으로 신뢰도 0(모든 토큰이 제외되므로 별도 "전부
  상용어" 분기 불필요), 혼합 선택("dkssud the")은 "the" 토큰만 제외되고 "dkssud"는 그대로
  후보로 남는다. 대신 상용어와 형태가 겹치는 단음절 한영타(go=해, to=새, did=양, **sp=네** …)는
  자동 제안이 안 뜬다 — 영문을 읽던 중 칩이 튀어나오는 오탐의 체감 비용이 훨씬 크기 때문에
  감수한 트레이드오프.
- **대량 fuzz 검증(2026-09, 실기기 없이 JVM 대량 실데이터로 검증)**: 영어 사전 37만 단어·
  네이버 영화리뷰 20만 문장·빈도 상위 1만 단어로 알고리즘을 실증 검증해 결함 2개를 발견·수정했다.
  ① *혼합 감지 미탐*: 선택 전체를 하나로 합쳐 판정하면, 진짜 한영타 옆에 조합 실패하는 다른
  라틴 조각(다른 두문자어 "SF"/"TV"/"CG"/"OO", `dvd`/`ost` 같은 실제 영단어, URL 등)이 있을 때
  그 조각들의 조합 실패가 신호를 희석시켜 신뢰도가 임계값 밑으로 떨어졌다(실제 문장 500개 뒤에
  `dkssud` 를 붙인 검증에서 31건 미탐). 위 토큰별 최댓값 방식으로 100% 감지로 개선. ②
  *단위 약어 오탐*: 토큰별 판정으로 바꾸자 "1cm" 처럼 숫자가 바로 붙은 짧은 단위 약어가 그
  자체로 최고 신뢰도를 내는 새 오탐이 나왔다 — 스톱워드 비교용 토큰 텍스트에 숫자가 섞여
  (`"1cm"`) 사전(`cm`)과 일치하지 않은 게 원인이라, 토큰의 글자만 모은 별도 버퍼로 비교하도록
  수정. **사전 기반 접근의 근본적 한계**: 빈도 상위 1만 단어 검증에서 스톱워드 보강 전 6.3%
  (627개)가 오탐이었다 — `ENGLISH_STOPWORDS_FREQUENT` 로 그 범위는 해소했지만, 이 범위를 벗어난
  어휘(전문용어·사어·신조어 등)에서는 여전히 오탐 여지가 있었다(전체 영어 사전 37만 단어
  기준으론 스톱워드 확장 직후에도 약 4%(14,982개)가 스톱워드 밖에서 오탐).
- **bigram 통계 안전망(`englishnessScore`, 2026-09 추가)**: 위 "근본적 한계"는 사전 확장만으로는
  구조적으로 못 없앤다 — 오탐이 나올 때마다 그 단어를 스톱워드에 손으로 추가하는 건 무한히
  반복되고 코드의 예외 목록만 계속 커진다. 그래서 정확 일치 스톱워드에 **통계적 안전망을
  추가**했다: 영어 사전 37만 단어에서 계산한 알파벳 26×26 bigram 로그확률표(라플라스 스무딩,
  676칸을 91단계로 양자화해 압축 문자열 하나로 내장)를 두고, 토큰을 이루는 연속 두 글자들의
  평균 로그확률("영어스러움" 점수)이 임계값(-8.5, 아래 보정 근거 참조) 이상이면 실제 영단어로
  보고 억제한다 — `dkssud` 의 `dk`/`ks`/`su` 처럼 영어에서 드문 조합은 점수가 낮고, `work`의
  `wo`/`or`/`rk` 처럼 흔한 조합은 높다. **사전 멤버십과 달리 한 번도 본 적 없는 단어에도
  일반화**되므로 향후 오탐마다 스톱워드를 추가해야 하는 부담을 크게 줄인다(전체 사전 37만 단어
  기준 잔여 오탐 14,982→**3,564개로 76% 감소**, 임계값 -8.5). ⚠️ 기존 스톱워드를 **대체하지는
  않는다** — `work`/`wow`/`keyboard`/`sp` 처럼 이미 알려진 고위험 단어의 상당수(빈도 위험 단어
  627개 중 63.2%)는 이 안전한 임계값에서도 통계로 못 잡는다(실제 단어치고 흔치 않은 자모
  배열이라 오히려 정확 일치가 더 정확함). 두 메커니즘은 상호 보완적이라 **OR 조건**(정확 일치
  스톱워드 또는 bigram 임계값 이상)으로 함께 쓴다. 임계값 -8.5는 네이버 영화리뷰 20만 문장에서
  뽑은 실제 한글 단어 34만개를 `convertKorToEng()` 로 되돌린 "진짜 한영타" 샘플로 보정 —
  이 지점에서 진짜 한영타 오억제(미탐 증가)는 0.29%(1004/347595)에 그친다(이미 감수 중인
  단음절 스톱워드 트레이드오프보다 낮은 비용).
