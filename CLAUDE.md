...
- **영어 상용어 스톱리스트(필수)**: 두벌식에서 자음-모음-자음 배열이 되는 흔한 영단어는 한글로
  ...
  의 `ENGLISH_STOPWORDS`(고빈도 영단어 ~150개, `god`/`sos` 등 짧은 단어 포함)로 거른다 — 선택이
  상용어(들)로만 이뤄졌으면 신뢰도 0(임계값 설정과 무관하게 차단), 혼합 선택("dkssud the")은
  억제하지 않는다. 대신 상용어와 형태가 겹치는 단음절 한영타(go=해, to=새, did=양 …)는 자동
  제안이 안 뜬다 — 영문을 읽던 중 칩이 튀어나오는 오탐의 체감 비용이 훨씬 크기 때문에 감수한
  트레이드오프.
...
- 그래서 최대한 보수적으로 판정한다: 변환 결과의 **모든 토큰이 `ENGLISH_STOPWORDS` 사전에
