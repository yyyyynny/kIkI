...
- **동작**: 토글을 켜면 `LangSenseAccessibilityService.onKeyEvent`가 모든 물리 키 다운 이벤트를
  (모디파이어 포함) 작은 원형 버퍼(`KeyTriggerDiagnostics.BUFFER_SIZE`=16칸, 키를 누르고 있을 때의 반복 이벤트는
...
- **한계 1(버퍼가 밀려나는 경우, 2026-09 발견·수정)**: 원형 버퍼는 "최근 2초"가 아니라
  ...
  잡히던 결함) — 반복 이벤트를 애초에 기록하지 않고 버퍼 크기를 16으로 넉넉히 잡아 완화했다.
...
- **결과는 "확정 원인"이 아니라 "후보 목록"**: 창 안에 눌린 키를 전부 보여주므로, 그 순간 계속
  ...
  눌러 보며 좁혀 나가는 용도로 쓴다.
- **"터치 키보드 제외"(추가 기능 2)와의 관계 — 완전히 독립적**: 진단 캡처(`onKeyEvent`/
  `onLanguageChanged` 양쪽)는 `featuresEnabled()`(터치 키보드 제외가 관장하는 게이트) 체크보다
  **앞**에서 실행되고, 키 필터 구독 여부(`syncServiceInfo()`)도 `noFocusEnabled ||
  diagnosticKeyLoggingEnabled` 로만 결정돼 `excludeTouchKeyboard` 와 무관하다. 즉 터치 키보드
  제외를 켜서 화면 터치 키보드 사용 중 플래시/배지 등이 전부 꺼져 있어도, 진단 기록 자체는
  계속 동작한다 — "왜 전환됐는가"는 오버레이 표시 여부와 별개의 관심사이기 때문에 의도한 설계다.
