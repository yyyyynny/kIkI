...
2. 선택된 텍스트 추출
3. `TextSelectionMonitor` 가 선택에 한글이 섞여 있는지(`HangulConverter.containsHangul`)로 방향을
   먼저 정하고(이중 변환 방지), `HangulConverter.analyze()`(영→한) 또는 `.analyzeReverse()`(한→영)
   호출 — 한영타 신뢰도 판정
...
- 그래서 최대한 보수적으로 판정한다: 변환 결과의 **모든 토큰이 `ENGLISH_STOPWORDS` 사전에
  ...
  정방향과 같은 원칙).
- `TextSelectionMonitor.onSelectionChanged()` 가 선택 문자 구성으로 방향을 미리 정하므로, 한
  선택에 두 방향을 다 계산하는 이중 변환은 없다(저사양 원칙 유지).
