...
상시 배지를 **탭**하면(드래그와 구분) 배지 주위로 입체 비눗방울 오브가 부채꼴로 펼쳐지는 간편 메뉴.
원본 디자인은 `claude/intelligent-edison-kwQHu` 브랜치의 `design/orb_mockup.html` **목업 v2**(사용자 승인본).

### 구성
- `RadialOrbView`: 입체 비눗방울(오프셋 구체 그라데이션 + 안쪽 밝은 링/바깥 이중 링 + 우하단 림
  라이트 + 가장자리 무지갯빛 sheen(정적) + 상단 광택 + 외곽 bloom). 라벨은 방울 안 중앙(흰
  bold + 시안 글로우). 외부 리소스/이미지 없이 순수 드로잉. 상수는 전부 `RadialMenuStyle`.
- 앵커(배지)에는 원형 헤일로 + 4꼭지 별 스파클, 배지→오브 발광 스포크(목업 SVG 이식).
- **모션 원칙(불쾌한 골짜기 방지)**: 오브별 랜덤 부유·선 위 빛 점 제거. 부유는 전 오브 동일 주기
  (4.4s)+인덱스 위상의 잔잔한 물결만. "저사양 모드" ON 이면 펼친 뒤 완전 정적.
