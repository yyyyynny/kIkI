...
object HangulConverter {
    ...
    /**
     * 신뢰도 판정: 입력 문자열이 한영타일 가능성 반환 (0.0 ~ 1.0)  [개념 설명용 — 단순화됨]
     *
     * 핵심 신호는 아래 두 개를 곱하는 것:
...
> 추가돼 `TextSelectionMonitor.pickAnalysis()` 가 정방향을 먼저 보고, 약하고 한글이 섞여
> 있을 때만 역방향도 계산해 더 나은 쪽을 쓴다(혼합 선택도 놓치지 않기 위함 —
