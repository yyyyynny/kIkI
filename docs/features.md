...
상시 배지를 **탭**하면(드래그와 구분) 배지 주위로 발광하는 원형 오브 버튼이 부채꼴로 솟아오르는
간편 메뉴. (원신 푸리나 스킬 연출의 빛나는 파랑-보라 구체 스타일)

### 구성
- `OrbView`: `Canvas` 로 그린 발광 오브. 외부 리소스/이미지 없이 순수 드로잉.
  - 본체: `RadialGradient`(중심 밝은 하늘색 `#BFE3FF` → 파랑 `#4D8BF5` → 보라 `#6B3CEA` → 외곽
    `#4A23B8`)로 입체 구체.
  - 발광/그림자: `BlurMaskFilter` 외곽 글로우(`#9959B0FF`) + 아래로 오프셋된 부드러운 drop shadow
    → 떠 있는 느낌.
  - 하이라이트: 상단 specular(흰 radial)로 광택, 중앙 라벨(흰 글씨 + 그림자).
  - 저사양 라이트 모드(`Prefs.orbGlowEnabled=false`): `BlurMaskFilter` 없이 얇은 반투명 링으로
    글로우를 대체해 성능 저하 0.
- 애니메이션(모두 `SpringAnimation` 물리 기반, `androidx.dynamicanimation`):
  - 등장: 아래에서 위로 스프링 바운스(버튼별 스태거)로 "톡톡" 솟아오름.
  - 드래그: 손가락을 탄성 있게 추종(스프링 목표를 손가락 위치로 갱신), 놓으면 제자리로 스프링 복귀.
  - 탭: 살짝 눌리는 scale down 피드백 후 동작.
  - 소멸: 페이드아웃 + 축소되며 사라짐.
- `QuickMenuOverlayView`: 전체화면 반투명 스크림(`#66000000`) 오버레이.
  - 윈도우 플래그: `TYPE_APPLICATION_OVERLAY | FLAG_NOT_FOCUSABLE | FLAG_LAYOUT_IN_SCREEN`
    (터치는 받되 키 포커스는 안 가져감), `windowAnimations=0`(연출은 뷰 애니메이션으로 직접 처리).
  - 배치: 앵커(배지 중심)에서 화면 중앙을 향하는 각도를 기준으로 약 150° 부채꼴, 반경 ≈140dp,
    각 버튼은 화면 밖으로 안 나가게 clamp.
  - 닫힘: 빈 곳(스크림) 탭 또는 항목 탭 → 페이드+축소 연출 후 제거. 배지가 사라지거나 서비스
    정리 시 함께 제거.
