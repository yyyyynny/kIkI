...
| # | 기능 | 핵심 구현체 |
|---|------|------------|
| 1 | 언어 전환 감지 플래시 | `ImeStateDetector` + `FlashOverlayView` |
| 2 | 상시 언어 표시 배지(드래그 이동) | `BadgeOverlayView` |
...
2. **접근성 서비스** 에서 LangSense 활성화
3. (권장) **배터리 최적화 예외** 등록
4. 앱 내 **설정** 에서 지원 언어/플래시 색상·속도·횟수 등 조정 (변경 즉시 적용)
